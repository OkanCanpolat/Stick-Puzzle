...
public class U_Up : IShapePlacementController
{
    ...
    public bool CanBePlaced()
    {
        for (int column = 0; column < gridManager.HorizontalLines.GetLength(0); column++)
        {
            for (int row = 0; row < gridManager.HorizontalLines.GetLength(1); row++)
            {
                ...
                Line verticalLine = gridManager.GetVerticalLine(column, row - 1);
                Line verticalLine2 = gridManager.GetVerticalLine(column +1 , row - 1);
