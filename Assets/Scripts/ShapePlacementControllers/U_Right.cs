...
public class U_Right : IShapePlacementController
{
    ...
    public bool CanBePlaced()
    {
        for (int column = 0; column < gridManager.VerticalLines.GetLength(0); column++)
        {
            for (int row = 0; row < gridManager.VerticalLines.GetLength(1); row++)
            {
                ...
                Line horizontalLine = gridManager.GetHorizontalLine(column - 1, row);
                Line horizontalLine2 = gridManager.GetHorizontalLine(column - 1, row + 1);
